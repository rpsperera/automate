...
import com.rpsperera.automation.automate_common.command_base.RestDelete;
...
public class APIDelete extends APIBase<APIDelete> implements RestDelete<APIDelete> {

    private static final Logger LOGGER = LoggerFactory.getLogger(APIDelete.class);

    @Override
    @Retry
    public ResponseDTO delete() throws AutomateException {
        try {
            return this.initAndSend();
        } catch (Exception e) {
            ExceptionUtils.generateAndThrowCommandException(e, Command.DELETE);
        }
        return new ResponseDTO();
    }

    /**
     * Use this method to execute a delete api with basic Authentication.
     *
    ...
    @Override
    @Retry
    public ResponseDTO delete(String username, String password) throws AutomateException {
        try {
    ...
    @Override
    @Retry
    public ResponseDTO delete(String identityMaterialPath, String identityProtocol, String identityMaterialPassword, SecureRandom secureRandom) throws AutomateException {
        try {
            ...
            if (e.getMessage().contains("PKIX")) {
                ...
                return deleteForSelfSignedCerts(identityMaterialPath, identityProtocol, identityMaterialPassword, secureRandom);
            }
            ExceptionUtils.generateAndThrowCommandException(e, Command.DELETE);
        }
        return new ResponseDTO();
    ...
    @Override
    @Retry
    public ResponseDTO deleteForSelfSignedCerts(String identityMaterialPath, String identityProtocol, String identityMaterialPassword, SecureRandom secureRandom) throws AutomateException {
        try {
            this.reset();
    ...
    protected ResponseDTO initAndSend() throws AutomateException, URISyntaxException, IOException, InterruptedException {
        this.init();
        HttpRequest request = this.request.DELETE().build();
